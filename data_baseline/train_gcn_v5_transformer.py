"""
train_gcn_v5_transformer.py - Version avec TransformerConv

TransformerConv applique l'attention multi-head de type Transformer sur les graphes.
Plus simple que GPS mais efficace.
"""
...
class MolGNN(nn.Module):
    """
    TransformerConv: attention multi-head sur le voisinage du graphe.
    Supporte edge_attr nativement.
    """
    def __init__(self, hidden_dim=HIDDEN_DIM, out_dim=768, 
        ...
        for _ in range(num_layers):
            conv = TransformerConv(
                ...
                beta=True,    # Learn skip connection weight
            )
    ...
    def forward(self, batch: Batch):
        ...
        for conv, bn in zip(self.convs, self.batch_norms):
            ...
            h_new = bn(h_new)
            h_new = F.relu(h_new)
            h_new = F.dropout(h_new, p=self.dropout if hasattr(self, 'dropout') else 0.1, training=self.training)
...
def main():
    print("=" * 60)
    print("TRAINING MolGNN v5 - TransformerConv")
    print("=" * 60)
    ...
    for ep in range(EPOCHS):
        ...
        if val_scores.get('MRR', 0) > best_mrr:
            ...
            torch.save(mol_enc.state_dict(), str(base_path / "model_checkpoint_v5_transformer.pt"))
